 ...
 * @param {object} config - ari configuration object
 * @returns {machina.Fsm} fsm - an finite state machine instance
 */
function fsm(sounds, channel, config) {
    ...
    states : {
      ...
      'init' : {
        _onEnter: function() {
          ...
          ari.getClient(config.url, config.username, config.password,
        ...
        next: function() {
          ...
          if (this.sound) {
            if (this.stopped && this.sound.skipable) {
              ...
              channel.play({media: this.sound.sound}, this.playback)
                .catch(function(err) {
                  self.emit('Error', err);
                  self.removeHangupHandler();
                });
 ...
 */
function createPrompt(sounds, channel, config) {
  var state = fsm(sounds, channel, config);
...
module.exports = function(config) {
  return {
    create: function(sounds, channel) {
      return createPrompt(sounds, channel, config);
    }
  };
};
